...
function typecheck(t: Term, tyEnv: TypeEnv): Type {
    switch (t.tag) {
        ...
        case "var": {
            if (!(t.name in tyEnv)) throw `unknown variable: ${t.name}`;
            return tyEnv[t.name];
        }
        case "func": {
        ...
        case "call": {
            ...
            if (t.args.length !== funcTy.params.length) throw "wrong number of arguments";
            for (let i = 0; i < t.args.length; i++) {
                const argTy = typecheck(t.args[i], tyEnv);
                if (!typeEq(argTy, funcTy.params[i].type)) {
                    throw `parameter type mismatch: [${i}]: expected ${funcTy.params[i].type.tag}, got ${argTy.tag}`;
                }
            }
